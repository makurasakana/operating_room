...
export class MenuComponent implements OnInit {
  ...
  menuNodeOnClick(event: NzFormatEmitEvent): void {
    ...
    this.http.get(`/menu/configDetail/${data.id}`).subscribe(
      (res: any) => {
        this.dataOnLoading = false;
        if (res.status === 0) {
          this.menuSelectData = res.data;

          const permissionList = deepCopy(this.permissionList) as any[];
  ...
  handleModalOk() {
    if (this.createMenuData_title) {
      ...
      this.dataOnLoading = true;
      this.http.post('/menu/create', data, {}).subscribe(
        (res: any) => {
          this.dataOnLoading = false;
          if (res.status === 0) {
  ...
  edit() {
    ...
    this.http.post('/menu/edit', data, {}).subscribe(
      (res: any) => {
        this.dataOnLoading = false;
        if (res.status === 0) {
  ...
  delete() {
    if (this.menuSelectData.menu.id) {
      this.dataOnLoading = true;
      this.http.get(`/menu/delete/${this.menuSelectData.menu.id}`).subscribe(
        (res: any) => {
          this.dataOnLoading = false;
          if (res.status === 0) {
            this.emptyTreeSelect();
  ...
  cancelPermission(node: NzTreeNode) {
    ...
    let target = node.key;
    this.dataOnLoading = true;
    this.http.get(`/permission/cancel/${resource}/${target}`).subscribe(
      (res: any) => {
        this.dataOnLoading = false;
        if (res.status === 0) {
  ...
  allowPermission(node: NzTreeNode) {
    ...
    let target = node.key;
    this.dataOnLoading = true;
    this.http.get(`/permission/allow/${resource}/${target}`).subscribe(
      (res: any) => {
        this.dataOnLoading = false;
        if (res.status === 0) {
  ...
  private initMenu() {
    this.dataOnLoading = true;
    this.http.get('/menu/configData').subscribe(
      (res: any) => {
        this.dataOnLoading = false;
